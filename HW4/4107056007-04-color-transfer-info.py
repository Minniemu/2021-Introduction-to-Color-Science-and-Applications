...
def color_transfer(source, target, clip=True, preserve_paper=True):
	...
	src = (lMeanSrc, lStdSrc, aMeanSrc, aStdSrc, bMeanSrc, bStdSrc) = image_stats(source)
	tar = (lMeanTar, lStdTar, aMeanTar, aStdTar, bMeanTar, bStdTar) = image_stats(target)
	
	#print(src)
	#src
	for i in range(0,len(src)):
		if i%2==0: #偶數
			src_1.append(round(src[i],4))
	for i in range(0,len(src)):
		if i%2==1: #奇數
			src_1.append(round(src[i],4))	
	print("src =",src_1)
