...
def color_transfer(result, clip=True, preserve_paper=True):
	...
	#rst = (lMeanRst, aMeanRst, aStdRst, bMeanRst, bStdRst) = image_stats(result)
	(lMeanSrc, lStdSrc, aMeanSrc, aStdSrc, bMeanSrc, bStdSrc) = (float(content[0]),float(content[3]),float(content[1]),float(content[4]),float(content[2]),float(content[5]))
	(lMeanTar, lStdTar, aMeanTar, aStdTar, bMeanTar, bStdTar) = (float(content[6]),float(content[9]),float(content[7]),float(content[10]),float(content[8]),float(content[11]))
	#write_file
	'''for i in range(0,len(src_1)):
		f.write(str(src_1[i]))
	for i in tar_1:
		f.write(str(i))'''
...
f = open("sideinfodeci.txt.","a")
with open("sideinfodeci.txt") as f:
    content = f.readlines()
...
# to the target image
transfer = color_transfer(source, clip=args["clip"], preserve_paper=args["preservePaper"])
#write_file
'''for i in range(0,len(src_1)):
	f.write(str(src_1[i]))
for i in tar_1:
